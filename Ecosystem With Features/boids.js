function Boid(id, loc){
  ...
  this.flock = function(boids){
    ...
    sep.mult(document.getElementById('sliderSep').value/10);
    ali.mult(document.getElementById('sliderAli').value/100);
    coh.mult(document.getElementById('sliderCoh').value/100);
...
Boid.prototype.seperate = function(boids){
  var desiredSep = document.getElementById('sliderSepRad').value;
...
Boid.prototype.cohesion = function(boids){
  var neighborDist = document.getElementById('sliderCohRad').value;
  var sum = new JSVector(0, 0);
  var count = 0;
  for (var i = 0; i < boids.length; i++){
    var distance = this.loc.distance(boids[i].loc);
